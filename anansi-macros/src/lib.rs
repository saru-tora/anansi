...
#[proc_macro_attribute]
pub fn component(args: proc_macro::TokenStream, input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let args = parse_macro_input!(args as SchemaArgs);
    let s = &args.vars[0];
    let f = parse_macro_input!(input as ItemFn);
    let component = quote!{#s};

    if f.sig.ident != "init" {
        panic!("expected `init` function for component");
    }
    let (prop_ident, props, properties) = match f.sig.inputs.first().unwrap() {
        syn::FnArg::Typed(pt) => {
            let p = match &*pt.pat {
                syn::Pat::Ident(pat_ident) => pat_ident.ident.clone(),
                _ => unimplemented!(),
            };
            (p, pt.pat.clone(), pt.ty.clone())
        }
        _ => unimplemented!(),
    };
    let mut comp = format_ident!("_");
    let mut init = vec![];
    let mut cbv = vec![];
    let mut start = vec![];
    let mut cbn: u8 = 0;
            ...
            Local(local) => {
                if let Some((_, expr)) = &local.init {
                    match &**expr {
                        Call(call) => {
                            match &*call.func {
                                Expr::Path(expr_path) => {
                                    ...
                                    if first.ident == "Self" || first.ident == component.to_string() {
                                        if expr_path.path.segments.last().unwrap().ident == "store" {
                                            match &local.pat {
                                                syn::Pat::Ident(pat_ident) => comp = pat_ident.ident.clone(),
                                                _ => unimplemented!(),
                                            }
                                        }
                                    }
                        ...
                        Macro(expr_macro) => if expr_macro.mac.path.segments.last().unwrap().ident == "async_callback" {
                            let name = match &local.pat {
                                syn::Pat::Ident(pat_ident) => pat_ident.ident.clone(),
                                _ => unimplemented!(),
                            };
                            let name = format_ident!("{}_{}", component.to_string().to_lowercase(), name);
                            let e = &expr_macro.mac.tokens;
                            let e: syn::Block = syn::parse_str(&quote! {{#e}}.to_string()).unwrap();
                            let add_proxy = AddProxy::new(comp.clone(), component.clone());
                            let blk = add_proxy.proxy(&e);
                            cbv.push(quote!{#cbn => #name(&mut #comp, &#props).await,});
                            let ns = name.to_string();
                            start.push(quote! {(#ns, #comp_mount, #cbn)});
                            cbn += 1;
                            let cb = quote! {
                                async fn #name(#comp: &mut #component, #props: &#properties) {
                                    #(#blk)*
                                }
                            };
                            callbacks.push(cb);
                ...
                init.push(quote!{#local});
            }
            Item(item) => {
                match item {
                    syn::Item::Macro(item_macro) => if item_macro.mac.path.segments.last().unwrap().ident == "rsx" {
                        let s = item_macro.mac.tokens.to_string();
                        let lower = component.to_string().to_lowercase();
                        let mut c_parser = CompParser {};
                        let parsed: syn::Expr = syn::parse_str(&c_parser.parse_rsx(&s)).unwrap();
                        let mut add_proxy = AddProxy::new(comp.clone(), component.clone());
                        let mut members = vec![];
                        rsx = add_proxy.check_expr(&parsed, &mut members);
                        add_proxy.vars.insert(prop_ident.clone());
                        let has_props = add_proxy.vars.get(&prop_ident).is_some();
                        ...
                        let mut parser = HtmlParser::comp(lower.to_string());
    ...
    let q = quote! {
        pub fn #comp_mount(state_val: Value, props_val: Value, _subs: Vec<Sub>, _receiver: async_channel::Receiver<u8>, _gtx: async_channel::Sender<anansi_aux::Cmd>, _node_id: String) {
            wasm_bindgen_futures::spawn_local(async move {
                let #props: #properties = serde_json::from_value(props_val).unwrap();
                let mut #comp = #component::new(state_val, _subs);
                while let Ok(_n) = _receiver.recv().await {
                    match _n  {
                        #(#cbv)*
                        _ => unimplemented!(),
                    }
        ...
        impl #component {
            pub const CB: &'static [(&'static str, fn(Value, Value, Vec<Sub>, async_channel::Receiver<u8>, async_channel::Sender<anansi_aux::Cmd>, String), u8)] = &[#(#start),*];
        }
    };
    q.into()
}
...
impl AddProxy {
    fn new(comp: Ident, component: TokenStream) -> Self {
        Self {comp, component, vars: HashSet::new()}
    }
    ...
    fn check_expr(&self, expr: &syn::Expr, members: &mut Vec<Ident>) -> TokenStream {
        match expr {
            Assign(assign) => {
    ...
    fn proxy(&self, b: &Block) -> Vec<TokenStream> {
        let mut qv = vec![];
        for stmt in &b.stmts {
            match stmt {
                Local(local) => {
...
fn attr_tuple(attr_str: &str) -> String {
    let mut s = String::new();
...
impl HtmlParser {
    ...
    fn tag(&mut self, view: &mut String, chars: &mut Chars) {
        ...
        if !name.starts_with(char::is_uppercase) {
            ...
            loop {
                ...
                if arg.starts_with('@') {
                    ...
                    let ar = if a.trim().starts_with('@') {
                        format!("anansi_aux::html_escape(&format!(\"{{}}\", {}))", a)
                    } else {
                        a.to_string()
                    };
            ...
            view.push_str(&format!("\");{}::init(<{0} as Component>::Properties {{{}}}, &mut _p);_c.push_str(\"", name, list));
        }
        if extra == '@' {
            self.at(view, chars);
        } else {
    ...
    fn at(&mut self, view: &mut String, chars: &mut Chars) {
        ...
        match s.trim() {
            ...
            "load" => {
                let (name, ex) = collect_name(chars);
                if name == "components" {
                    s = format!("let _p = anansi_aux::components::Pauser::new();");
                } else {
                    unimplemented!();
                }
                if ex == '{' {
            ...
            "link" => {
                let (av, segments) = html_attrs(&mut s, chars);
                let mut attrs = String::new();
                for attr in av {
            ...
            "href" => {
                let (_, segments) = html_attrs(&mut s, chars);
                let mut u = String::from(segments[0].clone());
                let mut v = String::from("{}");
                for segment in &segments[1..] {
                    u.push_str(&format!(", {}", segment));
...
fn variable(extra: &str, s: &mut String, chars: &mut Chars, view: &mut String) {
    if extra == "(" {
        ...
        view.push_str(&format!("_c.push_str(&anansi_aux::html_escape(&format!(\"{{}}\", {})));_c.push_str(\"{}", s, extra));
    }
}
